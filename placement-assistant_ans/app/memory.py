    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        ...
        with self.transaction() as c:
            ...
            c.execute(
                ...
                f"attempts = ?, started_at = COALESCE(started_at, {NOW_SQL}) WHERE id = ?",
    ...
    def fail_attempt(self, run_id: str, worker_id: str, error_code: str, retryable: bool,
        ...
        with self.transaction() as c:
            ...
            if attempts < max_attempts:
                next_avail = now + backoff_seconds * (2 ** (attempts - 1))
                c.execute("UPDATE run SET status = 'queued', available_at = ?, error_code = ?, "
                          "lease_owner = NULL, lease_until = NULL WHERE id = ?",
                          (next_avail, error_code, run_id))
                return "queued"
